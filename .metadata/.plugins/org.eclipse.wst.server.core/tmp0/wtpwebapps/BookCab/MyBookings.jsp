...
<div>
	...
	<c:when test="${mybookings ne null || !empty mybookings }">
	<table border = '1'>
	<tr>
	<th>User Name</th>
	<th>Car Number</th>
	...
	<c:forEach var = "bookings" items="${books }">
			<tr>
				<td>${bookings.userName}</td>
				<td>${bookings.carNumber}</td>
				<td>${bookings.driverName}</td>
				<td>${bookings.carColour}</td>
				<td>${bookings.fromAddress}</td>
				<td>${bookings.toAddress}</td>
		...
		<c:if test="${mybookings eq null || empty mybookings }">
		<h1>We are sorry <br>No Bookings are available</h1>
		</c:if>
		</c:when>
		<c:otherwise>
